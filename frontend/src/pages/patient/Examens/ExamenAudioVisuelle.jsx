...
export default function ExamenAudioVisuelleDetails() {
  const user = useUser();
  const { id } = useParams();
  ...
  const fetchData = async (url) => {
    ...
    try {
      ...
      console.error("Error fetching Examen Audio-Visuelle:", err);
      setError("Impossible de charger l'examen audio-visuel.");
      setExamen(null);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchData(`/api/examens/auditif/patient/${id}`);
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    try {
      await instance.put(`/api/examens/auditif/patient/${id}`, examen);
      setIsEditMode(false);
      fetchData(`/api/examens/auditif/patient/${id}`);
    } catch (err) {
      console.error("Error saving Examen Audio-Visuelle:", err);
  ...
  const fetchItem = async (dossierId) => {
    fetchData(`/api/examens/auditif/dossier/${dossierId}`);
    setIsHistory(true);
    setIsEditMode(false);
  ...
  const fields = [
    { key: "visionDePresDroit", label: "Vision de près (Droit)" },
