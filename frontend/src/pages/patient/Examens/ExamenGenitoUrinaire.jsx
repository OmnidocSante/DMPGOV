...
export default function ExamenGenitoUrinaire() {
  ...
  const fetchData = async (url) => {
    ...
    try {
      ...
      console.error("Error fetching Examen Genito-Urinaire:", err);
      setError("Impossible de charger l'examen genito-urinaire.");
      setExamen(null);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchData(`/api/examens/genito-urinaire/patient/${id}`);
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    try {
      await instance.put(`/api/examens/genito-urinaire/patient/${id}`, examen);
      setIsEditMode(false);
      fetchData(`/api/examens/genito-urinaire/patient/${id}`);
    } catch (err) {
      console.error("Error saving Examen Genito-Urinaire:", err);
  ...
  const fetchItem = async (dossierId) => {
    fetchData(`/api/examens/genito-urinaire/dossier/${dossierId}`);
    setIsHistory(true);
    setIsEditMode(false);
  ...
  const fields = [
    { key: "albumine", label: "Albumine" },
    { key: "sucre", label: "Sucre" },
    { key: "regles", label: "Règles" },
    { key: "ganglions", label: "Ganglions" },
    { key: "urinaire", label: "Urinaire" },
    { key: "rate", label: "Rate" },
    { key: "glandesEndocrines", label: "Glandes Endocrines" },
    { key: "neuroPsychisme", label: "Neuro-Psychisme" },
    { key: "tremblement", label: "Tremblement" },
    { key: "equilibre", label: "Equilibre" },
    { key: "reflexes", label: "Réflexes" },
    { key: "tendinaux", label: "Tendineux" },
    { key: "oculaires", label: "Oculaires" },
  ];
      ...
      <AnimatePresence>
        {showHistoryBanner && (
          ...
          >
            <Alert
              variant="default"
              className="bg-white/95 backdrop-blur-sm border border-purple-100 shadow-lg"
            >
              <HistoryIcon className="size-5 text-purple-600 shrink-0" />
