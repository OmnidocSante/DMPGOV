...
export default function PatientDetails() {
  ...
  const fetchData = async (url) => {
    ...
    try {
      ...
      console.error("Error fetching PatientDetail:", err);
  ...
  useEffect(() => {
    fetchData(`/api/patient-details/patient/${id}`);
  }, [id]);

  const handleFieldChange = (key, value) => {
    if (!isEditMode || isHistory || !patient) return;
    setPatient((prev) => ({ ...prev, [key]: value }));
  };

  const handleSave = async () => {
    if (!isEditMode || isHistory || !patient?.id) return;
    try {
      await instance.put(`/api/patient-details/patient/${id}`, patient);
      setIsEditMode(false);
      fetchData(`/api/patient-details/patient/${id}`);
    } catch (err) {
      console.error("Error saving PatientDetail:", err);
  ...
  const fetchItem = async (dossierId) => {
    fetchData(`/api/patient-details/dossier/${dossierId}`);
    setIsHistory(true);
    setIsEditMode(false);
  ...
  const fields = [
    { key: "niveauScholaire", label: "Niveau scolaire" },
    { key: "apprentissage", label: "Apprentissage" },
    { key: "CAP", label: "C.A.P" },
    { key: "gout", label: "Goût" },
    { key: "travailMonotone", label: "Travail monotone" },
    { key: "travailAttention", label: "Travail attention" },
    { key: "travailMachine", label: "Travail machine" },
    { key: "goutResponsabilites", label: "Goût responsabilités" },
  ];
      ...
      <AnimatePresence>
        {showHistoryBanner && (
          ...
          >
            <Alert
              variant="default"
              className="bg-white/95 backdrop-blur-sm border border-indigo-100 shadow-lg"
            >
              <HistoryIcon className="size-5 text-indigo-600 shrink-0" />
          ...
          <div className="flex gap-3">
            ...
            >
              ...
              <span className="text-sm font-medium text-gray-800">
                {isHistory ? "Cacher historique" : "Voir historique"}
              </span>
            </button>
            {isEditMode ? (
              <button
              ...
              <button
                ...
                disabled={isHistory}
                className="p-2 pl-4 rounded-lg flex items-center gap-2 hover:bg-indigo-50"
              >
