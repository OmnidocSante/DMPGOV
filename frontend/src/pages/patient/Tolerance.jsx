...
export default function ToleranceDetails() {
  const user = useUser();
  const { id } = useParams();
  ...
  const fetchData = async (url) => {
    ...
    try {
      ...
      setError("Impossible de charger la tolérance.");
      setTolerance(null);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchData(`/api/tolerances/patient/${id}`);
  }, [id]);

  const handleFieldChange = (key, value) => {
    if (!isEditMode || isHistory || !tolerance) return;
    setTolerance((prev) => ({ ...prev, [key]: value }));
  };

  const handleSave = async () => {
    if (!isEditMode || isHistory || !tolerance?.id) return;
    try {
      await instance.put(`/api/tolerances/patient/${id}`, tolerance);
      setIsEditMode(false);
      fetchData(`/api/tolerances/patient/${id}`);
    } catch (err) {
      console.error("Error saving Tolerance:", err);
      setError("Erreur lors de l'enregistrement de la tolérance.");
    }
  };
  ...
  const fetchItem = async (dossierId) => {
    fetchData(`/api/tolerances/dossier/${dossierId}`);
    setIsHistory(true);
    setIsEditMode(false);
  ...
  const fields = [
    { key: "trepidation", label: "Trépidation" },
    { key: "imtemperance", label: "Imtempérance" },
    { key: "temperance", label: "Tempérance" },
    { key: "irritationsTeguments", label: "Irritations des téguments" },
    { key: "irritantsRespiration", label: "Irritants de la respiration" },
    { key: "toxiques", label: "Toxiques" },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-800">Tolérance</h1>
        {user.role === "MEDECIN" && (
          <div className="flex gap-3">
            ...
            {isEditMode ? (
              ...
              <button
                ...
                disabled={isHistory}
                className="p-2 pl-4 rounded-lg flex items-center gap-2 hover:bg-blue-200"
              >
