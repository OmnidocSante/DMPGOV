...
export default function MusclesDetails() {
  const user = useUser();
  const { id } = useParams();
  ...
  const fetchData = async (url) => {
    ...
    try {
      ...
      setError("Impossible de charger l'examen musculaire.");
      setMuscles(null);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchData(`/api/muscles/patient/${id}`);
  }, [id]);

  const handleFieldChange = (key, value) => {
    if (!isEditMode || isHistory || !muscles) return;
    setMuscles(prev => ({ ...prev, [key]: value }));
  };

  const handleSave = async () => {
    if (!isEditMode || isHistory || !muscles?.id) return;
    try {
      await instance.put(`/api/muscles/patient/${id}`, muscles);
      setIsEditMode(false);
      fetchData(`/api/muscles/patient/${id}`);
    } catch (err) {
      console.error("Error saving Muscles:", err);
      setError("Erreur lors de l'enregistrement de l'examen musculaire.");
    }
  };
  ...
  const fetchItem = async (dossierId) => {
    fetchData(`/api/muscles/dossier/${dossierId}`);
    setIsHistory(true);
    setIsEditMode(false);
  ...
  const fields = [
    ...
    { key: 'stationDebout', label: 'Station debout' },
    { key: 'mouvementsTronc', label: 'Mouvements du tronc' },
    { key: 'mouvementsMiBassin', label: 'Mouvements du mi-bassin' },
    { key: 'valeurMusculaireTronc', label: 'Valeur musculaire du tronc' },
    { key: 'valeurMusculaireMiBassin', label: 'Valeur musculaire du mi-bassin' },
  ];
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }} className="p-6 bg-bay-of-many-50 min-h-screen">
      <AnimatePresence>
        {showHistoryBanner && (
          <motion.div className="w-full max-w-md fixed top-20 left-1/2 -translate-x-1/2 z-50" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
            <Alert variant="default" className="bg-white/95 backdrop-blur-sm border border-teal-100 shadow-lg">
              <HistoryIcon className="size-5 text-teal-600 shrink-0" />
              <AlertTitle className="text-sm font-semibold text-teal-800 mb-1">Historique Mode Active</AlertTitle>
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-800">Examen Musculaire</h1>
        {user.role === "MEDECIN" && (
          <div className="flex gap-3">
            ...
            {isEditMode ? (
              <button onClick={() => { fetchData(`/api/muscles/patient/${id}`); setIsEditMode(false); }} disabled={isHistory} className="p-2 pl-4 rounded-lg flex items-center gap-2 hover:bg-red-50">
                <Ban className="h-6 w-6 text-red-600" />
                <span className="text-sm font-medium text-red-800">Annuler</span>
              </button>
            ) : (
              <button onClick={() => setIsEditMode(true)} disabled={isHistory} className="p-2 pl-4 rounded-lg flex items-center gap-2 hover:bg-blue-50">
      ...
      <div className="space-y-6">
        {muscles && fields.map(({ key, label }) => (
          <motion.div key={key} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all">
            <h2 className="text-lg font-semibold text-gray-800 mb-2">{label}</h2>
            <input value={muscles[key] || ''} onChange={e => handleFieldChange(key, e.target.value)} placeholder={label} disabled={!isEditMode || isHistory} className={`w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 transition-all ${(isEditMode && !isHistory) ? 'border-teal-200 focus:ring-teal-300' : 'border-gray-200 focus:ring-gray-200 bg-gray-50 cursor-not-allowed'}`} />
          </motion.div>
        ))}
      </div>
    </motion.div>
