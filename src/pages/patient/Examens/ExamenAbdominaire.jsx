...
export default function ExamenAbdominaireDetails() {
  const user = useUser();
  const { id } = useParams();
  ...
  const fetchData = async (url) => {
    ...
    try {
      ...
      console.error("Error fetching ExamenAbdominaire:", err);
      setError("Impossible de charger l'examen abdominaire.");
      setExamen(null);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchData(`/api/examens/abdominaire/patient/${id}`);
  }, [id]);
  ...
  const handleSave = async () => {
    ...
    try {
      await instance.put(`/api/examens/abdominaire/patient/${id}`, examen);
      setIsEditMode(false);
      fetchData(`/api/examens/abdominaire/patient/${id}`);
    } catch (err) {
      console.error("Error saving ExamenAbdominaire:", err);
  ...
  const fetchItem = (dossierId) => {
    fetchData(`/api/examens/abdominaire/dossier/${dossierId}`);
    setIsHistory(true);
    setIsEditMode(false);
  ...
  if (loading) {
    return (
      ...
      >
        ...
        >
          <motion.div className="w-12 h-12 border-4 border-bay-of-many-400 border-t-transparent rounded-full animate-spin" />
  ...
  return (
    ...
    >
      <AnimatePresence>
        {showHistoryBanner && (
          ...
          >
            <Alert
              variant="default"
              className="bg-white/95 backdrop-blur-sm border border-bay-of-many-200 shadow-lg"
            >
              <HistoryIcon className="size-5 text-gray-600 shrink-0" />
              <AlertTitle className="text-sm font-semibold text-gray-800 mb-1">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-800">Examen Abdominaire</h1>
        {user.role === "MEDECIN" && (
          <div className="flex gap-3">
            ...
            {isEditMode ? (
              <button
                ...
                disabled={isHistory}
                className="p-2 pl-4 rounded-lg flex items-center gap-2 transition-all hover:bg-red-50"
              >
        ...
        <motion.div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md transition-all">
          ...
          <textarea
            ...
            className={`w-full px-4 py-3 border ${
              isEditMode && !isHistory ? "border-green-200" : "border-gray-200"
