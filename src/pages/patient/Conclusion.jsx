...
export default function ConclusionDetails() {
  const user = useUser();
  const { id } = useParams();
  ...
  const fetchData = async (url) => {
    ...
    try {
      ...
      setError("Impossible de charger la conclusion.");
      setConclusion(null);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => {
    fetchData(`/api/conclusions/patient/${id}`);
  }, [id]);

  const handleFieldChange = (key, value) => {
    if (!isEditMode || isHistory || !conclusion) return;
    setConclusion((prev) => ({ ...prev, [key]: value }));
  };

  const handleSave = async () => {
    if (!isEditMode || isHistory || !conclusion?.id) return;
    try {
      await instance.put(`/api/conclusions/patient/${id}`, conclusion);
      setIsEditMode(false);
      fetchData(`/api/conclusions/patient/${id}`);
    } catch (err) {
      console.error("Error saving Conclusion:", err);
      setError("Erreur lors de l'enregistrement de la conclusion.");
    }
  };
  ...
  const fetchItem = async (dossierId) => {
    fetchData(`/api/conclusions/dossier/${dossierId}`);
    setIsHistory(true);
    setIsEditMode(false);
  ...
  const fields = [
    { key: "horaireTravail", label: "Horaire de travail" },
    { key: "deplacements", label: "Déplacements" },
    { key: "bilangenerale", label: "Bilan général" },
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <h1 className="text-2xl font-bold text-gray-800">Conclusion</h1>
        {user.role === "MEDECIN" && (
          <div className="flex gap-3">
            ...
            {isEditMode ? (
              ...
              <button
                ...
                disabled={isHistory}
                className="p-2 pl-4 rounded-lg flex items-center gap-2 hover:bg-gray-200"
              >
